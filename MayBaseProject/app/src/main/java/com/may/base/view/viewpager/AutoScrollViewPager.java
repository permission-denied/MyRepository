 ...
 * <li>{@link #setInterval(int)} set auto scroll time in milliseconds, default is {@link #DEFAULT_INTERVAL}</li>
 * </ul>
 * <ul>
...
public class AutoScrollViewPager extends ViewPager {
    ...
    public static final int DEFAULT_SCROLL_ONCE_DURATION = 1000;
    ...
    private void init() {
        ...
        setScrollOnceDuration(DEFAULT_SCROLL_ONCE_DURATION);
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        //LogUtils.logD("AutoScrollViewPager", "onTouchEvent");
        if (forbidScroll) {
            return false;
        }
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                float moveX = ev.getX();
                float moveY = ev.getY();
                if (Math.abs(moveX - mLastMotionX) > mTouchSlop && Math.abs(moveX - mLastMotionX) > Math.abs(moveY - mLastMotionY)) {
                    isTouchDrag = true;
                }
                break;
            case MotionEvent.ACTION_UP:
